...
var update = (function(){
    ...
    function resolveCliCommand(cb){
        var _opts =  {};
        if(opts.cwd) _opts.cwd = opts.cwd;

        function resolveLocalCordova(){
            exec(localCordova + ' -v', _opts, function(err, stdout, stderr) {
                err = err || stderr;
                if(err){
        ...
        function resolveGlobalCordova(){
            exec(globalCordova + ' -v', _opts, function(err, stdout, stderr) {
                err = err || stderr;
                if(err){
        ...
        function resolveGlobalPhonegap(){
            exec(globalPhonegap + ' -v', _opts, function(err, stdout, stderr) {
                err = err || stderr;
                if(err){
    ...
    function updatePlugin(plugin, complete){
        ...
        function add(){
            ...
            if(plugin.source.type === "git"){
                ...
                pluginSource = plugin.source.path;
                ...
                if(pluginSource.match('@>=')){
                    pluginSource = pluginSource.split('@')[0] + '@*';
                }
            }else{
                logger.error("Unknown plugin source for '" + plugin.id+"'");
                return finish(-1);
            }
    ...
    function updatedPlugin(plugin, result){
        if(result === 0){
            logger.log("\nUpdated '"+plugin.id+"'"+" from "+(plugin.installed || "N/A")+" to "+plugin.target);
        }else{
    ...
    function forceRemove(pluginId){
        ...
        try{
            ...
            // remove from plugins/
            rmdirRfSync(path.join(local.PLUGINS_DIR, pluginId));
            ...
            var processPackageJson = function(){
                // remove from package.json
                local.readPackageJson(function(err, json){
                    if(err){
                        logger.error("Failed to read package.json to remove plugin entry for '"+pluginId+"': "+err);
                        return processConfigXml();
                    }
                    if(json.dependencies && json.dependencies[pluginId]){
                        delete json.dependencies[pluginId];
                        if(json.cordova && json.cordova.plugins && json.cordova.plugins[pluginId]){
                            delete json.cordova.plugins[pluginId];
                        }
                        local.writePackageJson(json, function(err){
                            if(err){
                                logger.error("Failed to write package.json after removing plugin entry for '"+pluginId+"': "+err);
                            }else{
                                logger.verbose("Removed plugin entry for '"+pluginId+"' from package.json");
                            }
                            processConfigXml();
                        });
                    }else{
    ...
    function remove (pluginId, callback){
    ...
    var rmdirRfSync; rmdirRfSync = function(_path) {
        var files = [];
        if( fs.existsSync(path.resolve(opts.cwd, _path)) ) {
            files = fs.readdirSync(path.resolve(opts.cwd, _path));
            files.forEach(function(file,index){
                var curPath = path.resolve(opts.cwd, _path + "/" + file);
            ...
            fs.rmdirSync(path.resolve(opts.cwd, _path));
        }
    };
        ...
        if(opts.updateMode === "auto" && outdated){
            ...
            pluginIds.forEach(function(pluginId){
                ...
                if(plugins[pluginId].status !== "newer-target" && (opts.target !== "config" || !opts.allowDowndate)){
                    valid = false;
                    window.alert(JSON.stringify(plugins[pluginId]));
    ...
    update.removeAll = function(plugins, _opts){
        opts = _opts;
